# Guide de configuration - Site C / Site 3

Le bloc professeur est `172.16.128.0/18` (`255.255.192.0`). Les commandes utilisent le découpage de travail `/24` par VLAN décrit dans `Plan_adressage_Site_C.md`. Faire valider ce découpage avant application.
...
```text
VLAN 10  172.16.128.1/24
VLAN 20  172.16.129.1/24
VLAN 30  172.16.130.1/24
VLAN 40  172.16.131.1/24
VLAN 50  172.16.132.1/24
VLAN 60  172.16.133.1/24
VLAN 70  172.16.134.1/24
VLAN 80  172.16.135.1/24
VLAN 99  172.16.136.1/24
```

Le serveur AD/DNS/DHCP de travail est `172.16.130.10`. Les SVI clientes utilisent `ip helper-address 172.16.130.10`.
...
Les flux Web passent par `172.16.138.10`. Proxmox 2 est `172.16.138.2`, Web 1 `.11` et Web 2 `.12` dans le plan de travail.
